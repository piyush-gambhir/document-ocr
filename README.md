...
Run `make test` for Python and SDK regressions. Use `make benchmark-public` and
`make benchmark-http` with the downloaded samples below for actual OCR accuracy
and API verification. Locally generated smoke fixtures are not used for the
active evaluation.
...
See [`AUDIT.md`](AUDIT.md) for historical implementation review. Current accuracy
claims use the [dataset-only evaluation](benchmarks/ALL_DOCUMENTS_EVALUATION.md).
...
Service config is generated by [`deploy/cloudrun/render_service.py`](deploy/cloudrun/render_service.py)
(2 GiB / 2 vCPU, `containerConcurrency: 1` since OCR is serialized). The `economy`
profile scales to zero; `DOCUMENT_OCR_PROFILE=warm` keeps one instance ready to
reduce cold starts. See [hosting profiles](HOSTING.md) for deployment settings.
...
Accuracy evaluation uses **downloaded dataset documents only**. See the
[dataset evaluation report](benchmarks/ALL_DOCUMENTS_EVALUATION.md) for matched
accuracy, latency, API results and remaining failures.

The current sample has 62 images: 39 labelled positives across non-Indian
passport books, South Dakota licences, US passport-card fronts and W-9 forms,
seven negative controls, and 16 observations without field truth. Some source
datasets contain publisher-created synthetic/specimen documents; these are
identified in the [coverage map](benchmarks/DOCUMENT_DATASET_COVERAGE.md).
They are externally supplied images and labels, not project-generated fixtures.
...
The [real HTTP evaluator](benchmarks/HTTP_EVALUATION.md) submits those same images
through the production server at one, two and four clients, checks PNG/JPEG/
image-only PDF inputs and hint/evidence options, and exercises batch, grouped
pages and queued jobs. Format conversions and repetitions do not add independent
people. Local HTTP timing is not a deployed-worker or serverless latency guarantee.

[Core CI](.github/workflows/ocr-evaluation.yml) and
[public HTTP CI](.github/workflows/ocr-http-public.yml) compare the same downloaded
samples, scorer, model files and environment across revisions. Correctness and
latency are separate gates; a regression pass can still have unmet absolute
targets. Locally generated image contracts and the agent-transcribed EAD specimen
are excluded from automatic accuracy evaluation and its headline results.

The [public dataset catalog](benchmarks/PUBLIC_DATASETS.md) and
[additional source review](benchmarks/ADDITIONAL_SOURCE_REVIEW.md) retain links
without downloading whole datasets. The bounded fetcher downloads only selected
images and labels with pinned checksums. **Only four supported profiles currently
have labelled dataset coverage. Real Indian passport and full Indian KYC accuracy
remain unmeasured.** Missing dataset samples remain explicit coverage gaps.
[Historical OCR results](benchmarks/PUBLIC_BASELINE.md) and
