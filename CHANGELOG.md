...
- **Synthetic sample**: `scripts/generate_synthetic_passport.py` renders a clean
  specimen passport with valid MRZ check digits; added to the benchmark
  (`synthetic-specimen-passport.png`) — replaces reliance on the watermarked
  samples for benchmarking. Reads back at 100% field + MRZ accuracy.
...
### Labelled accuracy dataset + end-to-end KYC benchmark
- New synthetic, labelled dataset under `sample-documents/<doc>/` (3 specimens
  each for PAN / Aadhaar / driving licence / voter ID), generated by
  `scripts/generate_synthetic_<doc>.py` on a shared `scripts/synth_common.py`
  (fonts, card canvas, degradations, manifest schema). All identities fictitious.
- New `benchmarks/document_accuracy.py` (`make benchmark-documents`) runs the
  full pipeline (image → OCR → fields) on every labelled image across a
  degradation sweep (blur / rotate / noise / JPEG / low-res) and scores routing +
  per-field accuracy against ground truth. Result: **100% routing and 100% field
  accuracy on clean images**; robust to blur/noise/JPEG (100%); ~82–88% under
  ±3° rotation (the preprocessor does not yet de-skew — see TODOS.md).

### Tests
- 90 → 244 passing Python tests (each KYC extractor has deterministic
  TextRegion-fixture coverage across many layouts/edge cases, plus a fast dataset
  + scoring guard); 12 → 24 TypeScript tests. Passport benchmark still green at
  100% status / field / MRZ / non-biodata accuracy.
