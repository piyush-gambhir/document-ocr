...
def recover_passport_mrz(image, regions: list[TextRegion], ocr) -> list[TextRegion]:
    ...
    anchors = [r for r in regions if _ANCHOR.fullmatch(_clean_mrz_text(r.text))
               ...
               and len(r.bbox) == 4]
    # Prefer longer line-two readings; at most two crop OCR calls per page.
    for anchor in sorted(anchors, key=lambda r: len(_clean_mrz_text(r.text)), reverse=True)[:2]:
        ...
        src = np.array([tl - 2.5 * vertical, tr - 2.5 * vertical,
                        br + .5 * vertical, bl + .5 * vertical], np.float32)
        w = min(1600, int(width))
        h = max(20, int(line_height * 4 * w / width))
        dst = np.array([[0, 0], [w - 1, 0], [w - 1, h - 1], [0, h - 1]], np.float32)
        transform = cv2.getPerspectiveTransform(src, dst)
        if abs(np.linalg.det(transform)) < 1e-12:
            continue
        crop = cv2.warpPerspective(image, transform, (w, h), borderValue=(255, 255, 255))
        inverse = np.linalg.inv(transform)
        recovered = []
        for region in ocr(crop):
